...
const homeDemo = `<section class="home-demo" aria-labelledby="home-demo-title"><div class="home-demo-inner"><div><p class="home-demo-kicker">iQabinet demo</p><h2 id="home-demo-title">See iQabinet in action</h2><p>A short walkthrough of the service.</p></div><div class="home-demo-video"><iframe title="iQabinet demo video" src="https://player.vimeo.com/video/104158849" loading="lazy" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe></div></div></section>`;
...
function cleanPage(html, route) {
  ...
  html = html.replace(/(<ul class="navigation menu fusion-navbar-nav">)/g, '$1<li class="menu-item menu-item-home"><a href="/">Home</a></li>');
  ...
  html = html.replace(/<\/head>/i, `${meta}\n<link rel="stylesheet" href="/site.css?v=20260910c">\n</head>`);
  if (route === "/contact/") {
    ...
    html = html.replace(/<div id="sidebar" style="float:right;">\s*<\/div>/i, "");
  }
  html = html.replace(/<div class="fusion-sharing-box share-box">[\s\S]*?<div class="fusion-clearfix"><\/div><\/div>\s*<\/div>/i, "");
  ...
  html = html.replace(/(<nav id="nav"[^>]*>)/i, '<button class="mobile-menu-toggle" type="button" aria-expanded="false" aria-controls="nav">Menu <span aria-hidden="true">☰</span></button>$1');
  html = html.replace(/<nav id="nav"/i, '<nav id="nav" aria-label="Primary navigation"');
  html = html.replace(/<body([^>]*)>/i, `<body$1><a class="skip-link" href="#main">Skip to content</a>`);
  html = html.replace(/<\/body>/i, '<script src="/site.js?v=20260910c" defer></script></body>');
  return html;
}
...
await appendFile(join(output, "site.js"), `
const menuButton = document.querySelector('.mobile-menu-toggle'); const primaryNav = document.querySelector('#nav'); menuButton?.addEventListener('click', () => { const open = primaryNav?.classList.toggle('mobile-menu-open'); menuButton.setAttribute('aria-expanded', String(Boolean(open))); });`);
await appendFile(join(output, "site.js"), `
document.querySelectorAll('.blog-medium').forEach((post) => { const article = post.querySelector('h2 a'); const image = post.querySelector('.image'); if (!article || !image) return; image.setAttribute('role', 'link'); image.setAttribute('tabindex', '0'); image.setAttribute('aria-label', article.textContent.trim()); const openArticle = () => { window.location.href = article.href; }; image.addEventListener('click', openArticle); image.addEventListener('keydown', (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openArticle(); } }); });`);
await writeFile(join(output, "site.css"), `.skip-link{position:absolute;left:-999px;top:0;background:#fff;color:#222;padding:10px;z-index:10000}.skip-link:focus{left:0}.archive-hero{min-height:520px;background:linear-gradient(90deg,rgba(255,255,255,.9) 0%,rgba(255,255,255,.62) 38%,rgba(255,255,255,.04) 66%),url('/media/home-hero.jpg') center/cover no-repeat;display:flex;align-items:center}.archive-hero-inner{width:1100px;margin:0 auto;padding:54px 30px}.archive-hero h1{margin:0;color:#409b00;font-size:60px;line-height:1.4}.archive-hero-actions{display:flex;gap:12px;margin-top:24px}.archive-hero-button{appearance:none;border:1px solid #409b00;border-radius:5px;background:#409b00;color:#fff;cursor:pointer;font:inherit;padding:12px 21px;text-decoration:none}.archive-hero-button:hover,.archive-hero-button:focus{background:transparent;color:#409b00}.home-demo{background:#f6f6f6;padding:72px 24px}.home-demo-inner{align-items:center;display:grid;gap:42px;grid-template-columns:minmax(220px,.7fr) minmax(0,1.3fr);margin:0 auto;max-width:1100px}.home-demo-kicker{color:#409b00;font-weight:700;margin:0 0 8px}.home-demo h2{color:#333;font-size:38px;line-height:1.15;margin:0 0 12px}.home-demo p{font-size:18px;line-height:1.5}.home-demo-video{aspect-ratio:16/9;background:#222;box-shadow:0 12px 32px rgba(0,0,0,.16)}.home-demo-video iframe{border:0;height:100%;width:100%}.video-dialog{border:0;box-shadow:0 18px 70px rgba(0,0,0,.42);max-width:min(960px,92vw);padding:0}.video-dialog::backdrop{background:rgba(0,0,0,.72)}.video-frame{width:min(960px,92vw);aspect-ratio:16/9}.video-frame iframe{border:0;height:100%;width:100%}.video-fallback{margin:0;padding:12px 16px;text-align:center}.video-fallback a{color:#409b00}.video-close{background:#fff;border:0;border-radius:50%;color:#222;cursor:pointer;font-size:30px;line-height:1;position:absolute;right:-14px;top:-14px;width:36px;height:36px}.contact-panel{margin:28px 0 48px;max-width:680px}.contact-panel details{background:#f6f6f6;border:1px solid #409b00;border-top-width:3px}.contact-panel summary{align-items:center;color:#333;cursor:pointer;display:flex;font-size:22px;font-weight:700;justify-content:space-between;list-style:none;padding:20px 24px}.contact-panel summary::-webkit-details-marker{display:none}.contact-panel summary::after{color:#409b00;content:'+';font-size:29px;font-weight:400;line-height:1}.contact-panel details[open] summary::after{content:'−'}.contact-panel-body{border-top:1px solid #d8d8d8;padding:4px 24px 24px}.contact-panel-body p{font-size:17px;line-height:1.6}.contact-linkedin{background:#409b00;border:1px solid #409b00;border-radius:4px;color:#fff;display:inline-block;font-weight:700;padding:11px 16px;text-decoration:none}.contact-linkedin:hover,.contact-linkedin:focus{background:#fff;color:#409b00}.single-navigation{align-items:center;display:flex!important;gap:12px;justify-content:space-between;margin:26px 0!important;padding:0!important}.single-navigation a{background:#409b00!important;border-radius:4px!important;color:#fff!important;display:inline-flex!important;align-items:center;gap:8px;justify-content:center;min-height:44px;padding:9px 14px!important;text-decoration:none!important}.single-navigation a[rel=next]{margin-left:auto}.blog-nav-arrow{font-size:21px;line-height:1}.single-navigation a[rel=prev]::before,.single-navigation a[rel=next]::after{content:none!important}.entry-read-more a::after{content:none!important}.article-ellipsis{color:#409b00;font-size:22px;font-weight:bold;text-decoration:none}.article-ellipsis:focus,.single-navigation a:focus,.archive-hero-button:focus,.video-close:focus,.contact-panel summary:focus,.contact-linkedin:focus{outline:3px solid #222;outline-offset:3px}@media (max-width:800px){#header .avada-row{align-items:center!important;display:flex!important;flex-direction:column!important;padding:20px 16px 12px!important}#header .logo{float:none!important;display:block!important;margin:0 auto 16px!important;text-align:center!important}#header .logo a{display:inline-block!important}#nav.nav-holder{display:block!important;float:none!important;height:auto!important;margin:0!important;text-align:center!important;width:100%!important}#nav.nav-holder .navigation{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;height:auto!important;margin:0!important;width:100%!important}#nav.nav-holder .navigation>li{display:block!important;float:none!important;margin:0!important;padding:0!important}#nav.nav-holder .navigation>li>a{display:block!important;font-size:13px!important;height:44px!important;line-height:44px!important;padding:0 4px!important;text-align:center!important;white-space:nowrap}#header-sticky{display:none!important}.archive-hero{min-height:420px;background-position:62% center}.archive-hero-inner{padding:42px 24px}.archive-hero h1{font-size:44px;line-height:1.2}.archive-hero-actions{flex-wrap:wrap}.home-demo{padding:48px 24px}.home-demo-inner{gap:24px;grid-template-columns:1fr}.home-demo h2{font-size:32px}.contact-panel summary{padding:18px}.contact-panel-body{padding:4px 18px 20px}.single-navigation{flex-wrap:wrap;margin:20px 0!important}.single-navigation a{flex:1 1 180px}.single-navigation a[rel=next]{margin-left:0}}@media (prefers-reduced-motion:reduce){*,*:before,*:after{scroll-behavior:auto!important;transition:none!important;animation:none!important}}`);
await appendFile(join(output, "site.css"), `.mobile-menu-toggle{display:none}@media (max-width:800px){#header .avada-row{padding:18px 16px!important}#header .logo{margin-bottom:10px!important}.mobile-menu-toggle{align-items:center;background:#409b00;border:0;border-radius:4px;color:#fff;display:inline-flex;font:700 14px/1 Arial,sans-serif;gap:8px;justify-content:center;min-height:44px;padding:0 16px}.mobile-menu-toggle span{font-size:18px}#nav.nav-holder{margin-top:10px!important}#nav.nav-holder .navigation{display:none!important}#nav.nav-holder.mobile-menu-open .navigation{display:grid!important;gap:1px;grid-template-columns:repeat(2,minmax(0,1fr))!important}#nav.nav-holder .navigation>li>a{background:#f3f3f3;height:44px!important;line-height:44px!important}.single-navigation{clear:both!important;display:grid!important;float:none!important;grid-template-columns:repeat(2,minmax(0,1fr));height:auto!important;margin:16px 0 28px!important;position:relative!important;width:100%!important}.single-navigation a{float:none!important;min-width:0!important;width:auto!important}.single-navigation a[rel=next]{margin-left:0!important}}`);
await appendFile(join(output, "site.css"), `.blog-medium .image{cursor:pointer}.blog-medium .image:focus{outline:3px solid #222;outline-offset:3px}@media (max-width:800px){#header .avada-row{position:relative!important}#header .logo{margin:0 auto!important}.mobile-menu-toggle{position:absolute;right:16px;top:20px}.blog-medium .image-extras{pointer-events:none!important}.page-id-86 #main{padding-top:24px!important}}`);
